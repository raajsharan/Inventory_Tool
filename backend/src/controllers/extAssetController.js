...
async function downloadTemplate(_req, res, next) {
  try {
    ...
    const ws = wb.addWorksheet('Ext Assets');
    ws.columns = COLUMNS.map(c => ({ header: c.header, key: c.key, width: c.width }));
    styleHeader(ws);

    ws.addRow({
      vm_name: 'EXT-PROD-WEB', os_hostname: 'ext-prod-web.corp.local', ip_address: '10.30.1.99',
      asset_type: 'Virtual Server', os_type: 'Linux', os_version: 'Ubuntu 22.04',
      assigned_user: 'Example User', department: 'IT Team', business_purpose: 'Example extended asset',
      server_status: 'Active', patching_type: 'Automatic', server_patch_type: 'Production',
      patching_schedule: 'Monthly', location: 'Remote DC', eol_status: 'Supported',
      serial_number: 'EXT-EX-001', ome_status: 'OK', hosted_ip: '', asset_tag: '',
    ...
    hdr.eachCell((c) => { c.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF065F46' } }; });
    hdr.height = 22;
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="ext-assets-template.xlsx"');
...
async function exportAssets(req, res, next) {
  try {
    ...
    await audit.log({ user: req.user, action: 'EXPORT', entityType: 'ext_asset', ipAddress: req.ip });
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', 'attachment; filename="ext-assets-export.xlsx"');
    await wb.xlsx.write(res);
    res.end();
  } catch (e) { next(e); }
}
...
async function importAssets(req, res, next) {
  try {
    ...
    const ws = wb.getWorksheet('Ext Assets') || wb.worksheets[0];
    ...
    await db.query(
      `INSERT INTO import_logs (filename, total_rows, success_rows, failed_rows, error_details, imported_by)
       VALUES ($1,$2,$3,$4,$5,$6)`,
      [req.file.originalname || 'ext-upload.xlsx', total, successes.length, failures.length, JSON.stringify(failures), req.user?.id || null]
    );
    await audit.log({ user: req.user, action: 'IMPORT', entityType: 'ext_asset', details: { total, success: successes.length, failed: failures.length }, ipAddress: req.ip });
